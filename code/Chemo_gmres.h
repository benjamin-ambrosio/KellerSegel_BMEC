...
//const double du=1.0;// in this version of the program du is a paramater that changes over time; same for b, gam and a
//const  double b=10.0, a=7, e=2.0, c=3, gam=0.1; // al=10 good value, gam=0.29 gives BMEC patterns
const double dv=10.0;// du=1 
//const double b=10.0, e=2.0, c=3, gam=0.1; // al=10 good value, gam=0.29 gives BMEC patterns 
double a=7, b=10.0,gam=0.29,du=1.0;

const double  e=2.0, c=3; // al=10 good value, gam=0.29 gives BMEC patterns 
...
// classe repr sentant la solution a  un instant t donne
class syst {
public:
  syst();
  ...
  syst& affect(double, double, double);
  syst& init(ifstream&, double);// Initial Condition from a file
 syst& init(ifstream&, double (*)(int,int));// Initial Condition from a file and a function
  void F(ofstream& );
...
double tau12quar(int i, int j)
{

return 0.1+ 0.005*alea[i][j][0];
//return tau7(i,j) + 0.2*alea[i][j][0]; //Good for BMEC
